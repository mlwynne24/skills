...
- **PowerShell compatibility:** Do not rely on `ConvertFrom-Json -AsHashtable`; older Windows PowerShell versions do not support it. Use `ConvertFrom-Json` with `PSObject` properties, or a short project-local script if structured JSON mutation gets awkward.
- **PowerShell wildcards:** Do not use `Copy-Item -LiteralPath "<path>\*"` when copying input files; `-LiteralPath` does not expand wildcards. Prefer `Copy-Item -Path "<path>\*" -Destination ...` when a wildcard is required.
- **Long Foundry calls:** interview extraction calls take minutes, especially over a multi-file batch. Set a generous shell timeout when invoking `uv run run.py`. The client is asynchronous; do not split a multi-file batch into one-at-a-time runs just because the first shell timeout was too low.
